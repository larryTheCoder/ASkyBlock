 ...
 */
@Log4j2
public class LevelCalcTask extends AsyncTask {

    private ASkyBlock plugin;
    private IslandData pendingQueue = null;

    public LevelCalcTask(IslandData islandDb) {
        if (SkyBlockEvent.eventCancellableCall(new IslandCalculateLevelEvent(islandDb))) return;

        plugin = ASkyBlock.get();
        pendingQueue = islandDb;
        Server.getInstance().getScheduler().scheduleAsyncTask(plugin, this);
    }
    ...
    public void onRun() {
        ...
        Map<String, Boolean> plPermission = plugin.getPermissionHandler().getPermissions(pendingQueue.getPlotOwner());
        ...
        for (Map.Entry<String, Boolean> pType : plPermission.entrySet()) {
            ...
            if ((!spl.isEmpty()) && Utils.isNumeric(spl)) {
                ...
                Utils.send("&cPlayer " + pendingQueue.getPlotOwner() + " has permission: " + type + " <-- the last part MUST be a number! Ignoring...");
            }
        }

        // Get the handicap
        int levelHandicap = pendingQueue.getLevelHandicap();
        // Get the death handicap
        int deathHandicap = 0;

        Level level = Server.getInstance().getLevelByName(pendingQueue.getLevelName());

        Set<FullChunk> chunkSnapshot = new HashSet<>();
        for (int x = pendingQueue.getMinProtectedX(); x < (pendingQueue.getMinProtectedX() + pendingQueue.getProtectionSize() + 16); x += 16) {
            for (int z = pendingQueue.getMinProtectedZ(); z < (pendingQueue.getMinProtectedZ() + pendingQueue.getProtectionSize() + 16); z += 16) {
        ...
        if (Settings.verboseCode) {
            ...
            Utils.sendDebug("Debugging chunk info for island " + pendingQueue.getIslandUniquePlotId());
        }

        WorldSettings settings = ASkyBlock.get().getSettings(pendingQueue.getLevelName());
        ...
        for (FullChunk chunk : chunkSnapshot) {
            for (int x = 0; x < 16; x++) {
                ...
                if (chunk.getX() * 16 + x < pendingQueue.getMinProtectedX() || chunk.getX() * 16 + x >= pendingQueue.getMinProtectedX() + pendingQueue.getProtectionSize()) {
                    continue;
                }

                for (int z = 0; z < 16; z++) {
                    // Check if the block coordinates is inside the protection zone and if not, don't count it
                    if (chunk.getZ() * 16 + z < pendingQueue.getMinProtectedZ() || chunk.getZ() * 16 + z >= pendingQueue.getMinProtectedZ() + pendingQueue.getProtectionSize()) {
                        continue;
                    }
        ...
        if (Settings.verboseCode) {
            ...
            reportLines.add("Level Log for island at " + pendingQueue.getCenter());
            reportLines.add("Asker is " + pendingQueue.getPlotOwner());
        ...
        final IslandData finalIsland = pendingQueue;
